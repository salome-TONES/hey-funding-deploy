...
// Balanced Weights Architecture (Total: 100)
const W = {
  territory: 25,
  entity: 25,
  sector: 0,
...
export function scoreProgram(p: Program, u: UserProfile): number {
  ...
  // 1. Territory Match (Including Regional Fallbacks)
  if (
    p.territory.includes(u.country) ||
    p.territory.includes("EU-wide") ||
    p.territory.includes("Global") ||
    (u.country === "Valencia" && p.territory.includes("Spain")) ||
    (u.country === "Murcia" && p.territory.includes("Spain")) ||
    (u.country === "Catalonia" && p.territory.includes("Spain"))
  ) {
    s += W.territory;
  }
  ...
  // 3. Sector & Core Activity Context Alignment
  if (p.sector === u.sector) {
    s += W.sector * 0.7;
    if (p.core_activity === u.core_activity) {
      s += W.sector * 0.3;
    }
  }

  // 4. Experience Tier Evaluation
  ...
  // 5. Duration Bucket Alignment
  if (durationBucket(p.project_duration) === u.preferred_duration) {
    s += W.duration;
  }

  // 6. Additive Capability Matrix
  const requiredTraits = [
    p.eu_partners_required,
    p.works_with_youth,
    p.can_mobilize_5_plus_youth,
    p.can_host_individuals,
  ].filter(Boolean).length;

  let matchingTraitsCount = 0;
  if (u.has_eu_partners && p.eu_partners_required) matchingTraitsCount++;
  if (u.works_with_youth && p.works_with_youth) matchingTraitsCount++;
  if (u.can_mobilize_5_plus_youth && p.can_mobilize_5_plus_youth) matchingTraitsCount++;
  if (u.can_host_individuals && p.can_host_individuals) matchingTraitsCount++;

  if (requiredTraits === 0) {
    s += W.capabilities;
  } else {
    s += (W.capabilities * matchingTraitsCount) / requiredTraits;
  }

  // 7. Dynamic Budget & Funding Match Engine
  const funding = typeof p.approx_funding === "number" ? p.approx_funding : parseFloat(String(p.approx_funding));

  if (isNaN(funding)) {
    s += W.funding;
  } else if (funding >= u.budget_target) {
    s += W.funding;
  } else if (funding >= u.budget_target * 0.5) {
    s += W.funding / 2;
  }

  // 8. Coordinator Operational Complexity Multiplier
  if (p.difficulty_level === "high" && u.coordinator_available) {
    s += 2;
  }
